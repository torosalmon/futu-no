...

{
  ...
  // 設定
  const header_change_trigger_y: number = 160; // ヘッダーUIを切り替えるY座標
  ...
  class THEME_COLOR_CONTROL {
    ...
    public change(): void {
      ...
      // 「body.show-drawer」 or 「body.show-search-form」が存在すればsub_colorへ変更
      if($body[0].classList.contains('show-drawer') || $body[0].classList.contains('show-search-form')) {
